# Arquitetura de Slides (Camada 2)
## Aula: Exame psíquico na pré-escola e na escola (0 a 5 e 6 a 12 anos)

Spec derivada de `01-direcao.md`, versão 3: duplo crivo aplicado (v2, ver `registro-crivos.md`) + integração das fontes F4 a F8 e do foco fechado pelo professor (DESCRIÇÃO do exame, não entrevista). Sem decisão visual: cores, fontes e diagramação são da Camada 3 (identidade Psiquiatria Prática).

Nota de faixa: o título oficial recebido dizia "0 a 5 e 5 a 12"; esta spec padroniza 0 a 5 e 6 a 12 (idade escolar conforme F2), e a criança de 5 anos pertence ao Ato 1. [PROPOSTA] confirmar no checkpoint.

**Regras de construção (valem para todos os slides):**
- Assertion-evidence: o TÍTULO é frase completa com a tese do slide; o CORPO traz só a evidência. Nada de bullets recitando o título.
- A sequência de títulos, lida sozinha, reconta a aula. Teste de aceitação: ler só os títulos em voz alta e verificar se a narrativa fecha.
- Foco em DESCRIÇÃO: nenhum slide termina em "o que perguntar" sem terminar em "o que escrever". Sempre que couber, o corpo fecha com a linha de prontuário correspondente.
- Detalhe fino (números com IC, autores, ressalvas longas) vai para as notas de apresentador e para o roteiro de estudo, nunca para o slide. No máximo UM número em destaque por slide.
- Todo vídeo tem slide-moldura ANTES (pergunta que orienta o olhar) e DEPOIS (síntese). O slide de vídeo só tem identificação mínima.
- Ancoragem: F1 a F5 por fonte + seção; vocabulário do curso pela ordem de precedência **G (glossário de arbitragem, `fontes/fundacao/00-GLOSSARIO-UNIFICADO-ARBITRAGEM.md`) > FND (16 extratos de fundação em `fontes/fundacao/`) > F6 a F8 (transcrições brutas)**. Conteúdo de fora das fontes marcado [EXT].
- Escopo terminológico do G §0: só entra no corpo o que tem destino NÚCLEO; RODAPÉ vira nota de uma linha; FORA não é renderizado (fica no glossário). Termos provisórios (vígil, conação/pragmatismo) aguardam os dois extratos que faltam.
- Jargão só entra com tradução de uma palavra no primeiro uso ("checking back, o olhar de checagem"; "toddler, a criança de 1 a 3 anos").
- Sem travessão em nenhum texto.

**Elementos estruturais recorrentes:**
- **Ancoragem "VOCÊS JÁ VIRAM" (interna, SEM selo renderizado):** decisão do professor no checkpoint: NENHUM selo de remissão aparece no deck. As marcações "selo VOCÊS JÁ VIRAM (...)" nos slides abaixo são ancoragem INTERNA desta spec: dizem ao construtor e ao apresentador qual extrato de fundação sustenta o domínio (FND cobre 16 domínios; só consciência-nível e conação/pragmatismo ficam sem extrato) e alimentam a narração ("como vocês viram na aula de..."), mas não viram elemento visual. A ponte com o adulto vive na fala e nas notas, não na tela.
- **Par "prontuário ruim / prontuário certo":** quatro aparições, identificadas só pelo nome: falso TDAH (S16), falso delírio (S31), falso TOD (S32), falso concretismo (S42).
- **Cenas de Bento:** narradas pelo professor, script nas notas. Bento nunca vira caso: é a régua do normal.
- **Contracena de vigilância:** moldura própria, sempre "o que tira o sono", nunca critério.

**Inventário de vídeos (3, todos de acervo público, nunca paciente do professor):**
| ID | Tipo | Spec do que o clipe precisa conter | Ato |
|----|------|-------------------------------------|-----|
| V1 | T3 cena natural | Birra de criança de 2 a 3 anos em local público, 60 a 90s, com o adulto entrando em corregulação (abaixar, voz baixa, conter). Congelar em 3 pontos: gatilho, pico, retorno. Técnica exibir-congelar-perguntar-reexibir. [EXT: clipe; doutrina em F3 §2 e §3.7.2] | 1 |
| V2 | T2 DNPM normal | Criança de 2 a 4 anos em brincar simbólico espontâneo, 20 a 40s: um objeto usado como outro, de preferência com fala dirigida ao brinquedo. [EXT: clipe; doutrina em F3 §3.4] | 1 |
| V3 | T1 quase-experimento | Atenção compartilhada em criança de 12 a 24 meses, 20 a 40s: apontar protodeclarativo com checagem do rosto do adulto e, se possível, contraste com apontar para pedir. Material de demonstração tipo M-CHAT serve. [EXT: paradigma; doutrina em F3 §3.7] | 1 |

Nota: a aula de adulto de pensamento (F8) já usa o vídeo piagetiano de conservação para ensinar concretismo normal; o S42 remete a esse vídeo em vez de repeti-lo.

**Total: 65 slides** (54 de conteúdo + 3 vídeos + 6 molduras + 2 cenas adicionais). Estimativa de fala: 75 a 90 min.
**Ordem de corte para 60 min:** (1º) bloco V1 completo, S13 a S15 (a técnica sobrevive na narração de S12); (2º) fundir S52+S53 (como perguntar + quando vira caso); (3º) fundir S40+S41 (consciência + orientação); (4º) fundir S21+S22 (conteúdo do brincar + brincar estagnado); (5º) absorver a cena S44 na narração de S45. Nunca cortar: V3, os dois prontuários (S34, S61), as vigilâncias (S33, S60), os dois slides do EU (S24, S46), o portal.

---

## PORTAL (S1 a S6)

**S1 · Capa**
- Título: **Exame psíquico na pré-escola e na escola (0 a 5 e 6 a 12 anos)** [PROPOSTA de padronização; título recebido dizia "5 a 12"]
- Subtítulo-tese: O mesmo mapa, em outra língua
- Corpo: professor e plataforma.

**S2 · Ponte com o adulto**
- Título: **Vocês já aprenderam o exame psíquico do adulto neste curso; o da criança não muda de mapa, muda de língua.**
- Corpo: o mapa de funções das aulas que o residente já viu (consciência, atenção, linguagem, afetividade, pensamento, sensopercepção, psicomotricidade, consciência do EU, juízo), com as capas/nomes das aulas de adulto do curso. Selo "VOCÊS JÁ VIRAM" apresentado aqui. Fontes: FND (os 16 extratos de fundação); G; F2 introdução.
- Narração: "nada disso vai deixar de ser examinado hoje; e o que vocês aprenderam lá, inclusive o examinador como instrumento, vale dobrado aqui".

**S3 · Tese**
- Título: **Na criança, o discurso vira brincar, o paciente vira díade e a norma vira idade: três traduções, o resto vocês já sabem.**
- Corpo: as três traduções em esquema (canal, paciente, régua). Fontes: F3 §3.4; F3 §3.7 e F1 achado 1; F1 achado 2.

**S4 · Regra de ouro**
- Título: **Antes de qualquer nome técnico, a regra de ouro: o exame registra o que você viu, não o que você concluiu.**
- Corpo: a cena do carrinho (F3 §2) em duas colunas: dado bruto vs inferência. A inferência só existe DEPOIS.
- Narração: inferência precoce fabrica estigma (F3 §2 e §5). O irmão da regra de ouro: uma sessão pode não ser representativa; quando a consulta não parecer representativa, o registro honesto termina em "reexaminar" (F1 achado 1 e recomendação 1).

**S5 · Trio de dados**
- Título: **Todo dado do prontuário tem dono: o que você observou, o que alguém relatou e o que você inferiu.**
- Corpo: trio de dados com exemplo de uma linha cada (observado / relatado com fonte nomeada / inferido). Regra: o campo exame psíquico só recebe o que o médico viu na sessão; relato de pais pertence à anamnese. Fontes: F2 introdução; F3 §4.1.

**S6 · Contrato + personagem**
- Título: **Vamos examinar a mesma criança duas vezes, aos 3 e aos 9 anos, e escrever os dois prontuários juntos.**
- Corpo: Bento em duas idades. Regras: Bento é normal do início ao fim; a aula termina quando o corpo dele começar a mudar; o produto da aula é o registro escrito.

## ATO 1 (0 a 5 anos) · A CONSULTA EM QUE O PACIENTE NÃO FALA (S7 a S35)

**S7 · Cena Bento 1**
- Título: **Bento, 3 anos e 7 meses, para na porta, agarra a perna da mãe e não tira os olhos da caixa de brinquedos.**
- Corpo: imagem única de consultório. Cena narrada (script nas notas).

**S8 · Observação começa antes**
- Título: **O exame começou antes do bom-dia: sala de espera, colo, separação e a entrada na sala já são dados.**
- Corpo: linha da consulta (sala de espera → cumprimento → separação → exploração → brincar → transições → saída) com "o que observar e registrar" em cada ponto. Fonte: F1 achado 3.
- Narração: o kit de sala que torna o exame possível: brinquedos genéricos, papel e lápis, peças de encaixe (F1 estrutura pré-escolar; F3 §3.9).

**S9 · Reação inicial e a régua**
- Título: **Hesitar diante do estranho é saúde; o alarme é a criança que congela ou a que trata um estranho como íntimo em dois minutos.**
- Corpo: espectro em três posições: exploração cautelosa referenciada / congelamento motor de alerta / desinibição social atípica. Linha de prontuário para cada uma. Fonte: F3 §3.1.

**S10 · Díade**
- Título: **Não existe bebê sozinho: quem entra em exame é a díade, e o cuidador é parte do campo, não ruído.**
- Corpo: selo VOCÊS JÁ VIRAM (FND afetividade: a afetividade é um modo de relação; aqui, a relação examinada é a díade). Três observáveis com linha de registro: base segura com checking back (o olhar de checagem), corregulação diádica, apego indiscriminado como alerta. Fontes: F3 §3.7.2; F1 domínio 2.

**S11 · Regulação e sensorial**
- Título: **O primeiro domínio da primeira infância é a regulação: como a criança transita entre estados e o que o corpo dela faz com o mundo sensorial.**
- Corpo: eixo estados (transições suaves vs abruptas; autoacalmar-se) + eixo sensorial (hiper-responsividade, hipo-responsividade, busca sensorial). Fontes: F3 §3.2; F1 ITMSE domínio III.
- Narração: DC:0-5 elevou regulação a categoria primária (uma frase).

**S12 · ITMSE (0 a 2 anos)**
- Título: **Para o bebê que ainda não brinca de faz de conta existe roteiro pronto: o exame de 0 a 36 meses é observação narrativa em dez domínios.**
- Corpo: o Infant-Toddler Mental Status Exam (ITMSE, de Benham), 10 domínios em uma linha cada. Remissão: apêndice A. Fontes: F1 seção ITMSE; F3 §1.

**S13 · Moldura-antes V1**
- Título: **Nesta cena, repare no que acontece imediatamente antes do grito e no que o adulto faz com o corpo.**
- Corpo: frame congelado. Anúncio da técnica: exibir, congelar, perguntar, reexibir.

**S14 · VÍDEO V1** (birra + corregulação, 60 a 90s; roteiro de pausas nas notas)

**S15 · Moldura-depois V1**
- Título: **Você acabou de examinar regulação, afeto e díade sem fazer uma pergunta: gatilho, pico, corregulação e retorno estão descritos.**
- Corpo: os 3 frames rotulados (gatilho / pico / retorno com corregulação) + a frase de prontuário descritiva correspondente. Fontes: F3 §2 e §3.7.2.

**S16 · Psicomotricidade + falso TDAH**
- Título: **O toddler, a criança de 1 a 3 anos, que não para quieto está obedecendo à idade, não a um diagnóstico.**
- Corpo: par prontuário ruim ("criança hiperativa, não para na cadeira") vs prontuário certo ("atividade exploratória vigorosa, deambulação ampla, curiosidade preservada, normativa para a idade"). E o que se descreve além do nível de atividade: estereotipias motoras (flapping, balanceio), achado de peso próprio no pré-escolar. Fontes: F3 §5.1 e §3.3; F1 domínio 3; G §A14.
- Narração: o que diferencia: atividade COM objetivo exploratório vs agitação desorganizada sem função. A leitura protomovimento do curso (a estereotipia como resíduo de sentido ou sentido em construção, FND psicomotricidade, uso próprio do professor) entra aqui como ponte com o desenvolvimento.

**S17 · Brincar = pensamento (forma)**
- Título: **Na criança pequena o brincar é o discurso: a forma do brincar é a forma do pensamento.**
- Corpo: selo VOCÊS JÁ VIRAM (FND pensamento: curso, forma e conteúdo; "quem estabelece a forma é o examinador", e aqui o examinador a estabelece olhando o brincar). Escada do brincar: sensório-motor (0 a 12m) → funcional (12 a 18m) → simbólico inicial (18m+) → simbólico complexo (30m+). Fontes: F3 §3.4 e §3.4.1; F1 domínio 13; FND pensamento.

**S18 · Moldura-antes V2**
- Título: **Repare no que o objeto vira nas mãos da criança.**
- Corpo: frame congelado.

**S19 · VÍDEO V2** (brincar simbólico espontâneo, 20 a 40s)

**S20 · Moldura-depois V2**
- Título: **Quando um bloco vira comida, a função simbólica está no lugar: esse é o "curso e forma" do pré-escolar.**
- Corpo: frase-síntese + linha de prontuário ("brincar simbólico estabelecido, com narrativa").

**S21 · Brincar (conteúdo)**
- Título: **O brincar também tem conteúdo: os temas que se repetem são as preocupações da criança postas na mesa.**
- Corpo: selo VOCÊS JÁ VIRAM (FND pensamento: conteúdo = o preenchimento). Três conteúdos que pedem registro: temas agressivos sem resolução, reencenação pós-traumática, pobreza de fantasia. Fontes: F3 §3.4.2; F1 domínio 13.

**S22 · Brincar estagnado (alerta)**
- Título: **O brincar que só repete é a perseveração da infância; o brincar que pula sem construir é a desagregação.**
- Corpo: selo VOCÊS JÁ VIRAM (FND pensamento: perseveração como "disco arranhado", desagregação como "pera aí, não entendi"; aqui os mesmos fenômenos aparecem no brincar). Dois padrões com linha de registro: brincar estagnado/circular; desagregação lúdica. Fontes: F3 §3.4.1; FND pensamento; G §A7.

**S23 · Linguagem**
- Título: **Ecolalia aos 2 anos é ensaio de fala; aos 4, com a melodia copiada da fala original e sem função, é dado de exame.**
- Corpo: selo VOCÊS JÁ VIRAM (FND linguagem: ecolalia/palilalia; prosódia; intenção comunicativa). Marcos: ecolalia normativa até por volta dos 3; reversão pronominal; intenção comunicativa pré-verbal como o que importa quando não há fala. Fontes: F1 domínio 5; F3 §3.8; FND linguagem.
- Nota: ecolalia no TEA citada como faixa (25 a 91%), nunca valor único (F1 domínio 5).

**S24 · Consciência do EU (pré-escolar)**
- Título: **Aos 3 anos o EU já dá as caras: no "eu" e "meu" da fala, no corpo desenhado e no menino que sabe que é menino.**
- Corpo: selo VOCÊS JÁ VIRAM (consciência do EU do exame adulto → aqui o EU emergente). O esperado com linha de registro: pronomes pessoais consistentes, autodescrição concreta e física ("eu sou grande", "eu gosto de brincar"), identidade de gênero emergente, criança que se inclui no desenho da família. Alertas: terceira pessoa persistente aos 4 anos, nenhuma autodescrição aos 4, não se incluir no desenho. Fonte: F4 (3-4 anos e 5-6 anos).
- Narração: constância de gênero ainda instável é normal aos 3 a 4; superestimar as próprias capacidades ("sou o melhor corredor") é normativo, não grandiosidade.

**S25 · Contracena TEA (o apontar)**
- Título: **Mais grave que a fala que atrasa é o apontar que nunca mostra: quem não divide o mundo precisa de avaliação, e já.**
- Corpo: contracena de vigilância. Par de imagens: apontar para pedir (protoimperativo, preservado) vs apontar para mostrar (protodeclarativo, o que falha). Janela do segundo ano, sem idade rígida. Fonte: F3 §3.7.1; F1 domínio 4.
- Narração OBRIGATÓRIA: atraso de fala TAMBÉM motiva avaliação (F3 §3.8); o título hierarquiza, não tranquiliza. Vigilância e encaminhamento, não diagnóstico.

**S26 · Moldura-antes V3**
- Título: **Nesta cena, conte quantas vezes a criança volta os olhos para o rosto do adulto.**
- Corpo: frame congelado.

**S27 · VÍDEO V3** (atenção compartilhada / apontar protodeclarativo, 20 a 40s)

**S28 · Moldura-depois V3**
- Título: **Apontar para pedir usa o adulto como ferramenta; apontar para mostrar divide um mundo: o triângulo olho-objeto-olho é o dado.**
- Corpo: o triângulo da atenção compartilhada sobre o frame + linha de registro. Fonte: F3 §3.7.1.

**S29 · Afeto na primeira infância**
- Título: **Vocês já sabem que o examinador é um dado do exame da afetividade; na criança pequena, ele é o principal instrumento.**
- Corpo: selo VOCÊS JÁ VIRAM (FND afetividade: ressonância, "o examinador é um dado", diagnóstico atmosférico). Três observáveis com linha de registro: sintonia (espelha o clima), ressonância (a criança saudável te contagia; o vazio contratransferencial é achado positivo), labilidade AFETIVA normativa do toddler (sempre com qualificador, G §2; só vira nota se desproporcional, disruptiva e sem retorno à linha de base). Fontes: FND afetividade; F3 §3.6; G §2 e §3.9.
- Narração: a ressonância é MÉTODO, não só conceito: estímulo emocional deliberado + observação da reciprocidade, com o brincar no lugar da "piadinha de entrevista" do adulto (G §3.9).

**S30 · Sensopercepção e fantasia**
- Título: **Monstro no escuro e amigo imaginário são arquitetura normal da idade; alucinação verdadeira aqui é raridade que pede investigação orgânica.**
- Corpo: três fenômenos em escada, cada um com linha de registro: ilusão fisiológica (corrigível ao acender a luz), amigo imaginário (lúdico, evocável, sem angústia persecutória), alucinação verdadeira suspeita (vigília plena, terror, desorganização → organicidade). Fontes: F3 §3.5; F1 domínio 9 e ITMSE domínio VI.

**S31 · Pensamento mágico + falso delírio**
- Título: **A boneca que chora de medo não é delírio: nessa idade o mundo é animado e os desejos têm poder.**
- Corpo: par prontuário ruim ("delírio bizarro persecutório") vs prontuário certo ("pensamento de formato mágico e animista, normativo para a fase pré-escolar"). Pensamento mágico normativo dos 2 aos 7. Fontes: F3 §5.1; F1 domínio 8; F2 pensamento.
- Narração: fantasia normal é reconhecida como faz de conta, evocável, de emoção positiva (F1 domínio 8). Declínio começa na escolarização e se completa por volta dos 10 (F1): resíduo mágico aos 9 não é psicose.

**S32 · Birra e oposição + falso TOD**
- Título: **Dizer não aos 3 anos é construção do ego; o que se pergunta à birra é intensidade, rigidez e recuperação, nunca a existência.**
- Corpo: par prontuário ruim ("comportamento persistentemente negativista e hostil") vs prontuário certo (descrição da cena + calibração: excede o esperado em intensidade e frequência? em mais de um ambiente? recupera depois?). Expandir a sigla: transtorno opositor desafiador (TOD). Fontes: F3 §5.1 e §3.3; F1 achado 2.

**S33 · Vigilância do Ato 1**
- Título: **Aos 3 anos, o que tira o sono: perder o que já tinha, congelar, não simbolizar, não mostrar e não sintonizar.**
- Corpo: contracena-síntese, cinco alertas em uma linha cada. Fontes: F1 recomendação 6; F3 §3.1, §3.4, §3.6, §3.7.
- Narração: a consulta que "não aconteceu" também é exame: a criança que chorou o tempo todo, não se separou, não brincou, se descreve exatamente assim, e se marca reavaliação (F1 achado 1).

**S34 · Prontuário 1**
- Título: **Assim se escreve o exame de Bento aos 3: uma narrativa do que foi visto, com a idade como régua explícita.**
- Corpo: o exame psíquico de Bento aos 3 anos e 7 meses, por extenso (8 a 12 linhas), no formato NARRATIVO da aula de estrutura do curso (prosa, primeira pessoa, o "como"; G §A20), com atribuição de fonte, sem rótulo, fechando com a ressalva de representatividade da sessão. Texto integral nas notas e no roteiro. Modelos: F1 redações-modelo; F3 §4; FND estrutura; G §A20.
- Narração: ler em voz alta, apontando cada tradução usada. Anunciar que a mesma observação tem uma segunda saída, a súmula (lista, terceira pessoa, o "quê"), ensinada no apêndice B.

**S35 · Tração**
- Título: **Bento cresceu, entrou na escola e agora responde por si. Quando o paciente começa a falar, o que muda no exame?**
- Corpo: Bento de mochila, porta da escola. Uma linha: "quase tudo, e quase nada".

## ATO 2 (6 a 12 anos) · A CONSULTA EM QUE O PACIENTE COMEÇA A RESPONDER (S36 a S61)

**S36 · Cena Bento 2**
- Título: **Bento, 9 anos, entra na frente da mãe, senta, e pergunta se pode mexer nos lápis.**
- Corpo: imagem única. Cena narrada (script nas notas).

**S37 · O que muda e o que fica**
- Título: **Na idade escolar o autorrelato chega, mas não aposenta a observação: a fala vira mais um canal, não o único.**
- Corpo: o que chega (autorrelato, pensamento operatório concreto, escola como terceira fonte) e o que fica (observação, desenho e jogo como canais). Fontes: F2 introdução; F1 estrutura por faixa.

**S38 · Momento a sós**
- Título: **Uma parte da consulta é a sós com a criança, sempre: é ali que moram o abuso, o risco e o que a lealdade cala.**
- Corpo: regra inegociável + o que só aparece a sós + a moldura do sigilo dita à criança: sigilo dentro dos limites de proteção à vida. E o destino no papel: o que a criança revela a sós entra no exame como relato dela, com fonte atribuída. Fontes: F2 introdução e risco; F1 domínios 12 e 14.

**S39 · Aparência e atitude**
- Título: **Atitude se descreve com cena, não com adjetivo: o "opositivo" precisa da situação que o provou.**
- Corpo: selo VOCÊS JÁ VIRAM (FND atitude e FND aparência, os "dois As" da primeira impressão na aula de estrutura). Vocabulário em uma linha (cooperativo, apático, opositivo, pueril, desconfiado) + regra: registrar sempre a situação que embasou. Fontes: FND atitude e aparência; F2 apresentação geral; F1 domínio 2; G §5.
- Narração: alerta de registro da aparência infantil (já problematizado no próprio FND de aparência): higiene e vestuário da criança medem o sistema de cuidado, não a criança; descrever sem juízo de valor (F2 apresentação geral).

**S40 · Consciência**
- Título: **Criança sonolenta ou confusa não é caso psiquiátrico até prova em contrário: rebaixamento de consciência aqui é emergência médica.**
- Corpo: uma frase de conduta (delirium, intoxicação, pós-ictal, TCE antes de qualquer leitura psiquiátrica). Fonte: F2 nível de consciência.

**S41 · Orientação ecológica**
- Título: **Oriente-se pela vida da criança, não pelo calendário: escola, série, professora e se amanhã tem aula.**
- Corpo: par de perguntas ruins vs calibradas + linha de registro ("orientada quanto à escola, série e rotina"). Fontes: F2 orientação; F1 domínio 10.

**S42 · Cognição + falso concretismo**
- Título: **O provérbio interpretado ao pé da letra aos 8 anos é Piaget, não esquizofrenia: o concreto é o normal da idade.**
- Corpo: par prontuário ruim ("concretismo, empobrecimento do pensamento") vs prontuário certo ("pensamento operatório concreto, normativo para a fase escolar"). Provérbios e abstração são inúteis antes da adolescência. Fontes: F2 exame cognitivo; F1 armadilhas; F8 (concretismo como normal do desenvolvimento).
- Narração: remeter ao vídeo das moedas que a turma viu na aula de pensamento do adulto (FND pensamento): o curso já ensinou que a criança do experimento não está doente; hoje fechamos o circuito. E a moldura desenvolvimental do FND de inteligência (janelas/período crítico, catch-up, o impacto da privação de estímulo, as "crianças da pandemia"; G §3.1) entra nas notas e no roteiro de estudo como o pano de fundo de por que a régua é a idade.

**S43 · Atenção**
- Título: **Atenção se descreve em dois eixos, fixar e mover: os 15 minutos de quebra-cabeça valem mais que qualquer pergunta.**
- Corpo: selo VOCÊS JÁ VIRAM (FND atenção). Eixos conforme a arbitragem A2/A15: tenacidade (fixar) e MOBILIDADE (mover o foco; descrever como hipermobilidade/hipomobilidade, nunca como "hiper/hipovigilância"). Três padrões com linha de registro: atenção preservada (normoprosexia), hipotenacidade com hipermobilidade (a distratibilidade), hipertenacidade com hipomobilidade (o foco que "fica surdo"). Fontes: FND atenção; F2 exame cognitivo; G §A2 e §A15.
- Narração: "vigilância" sozinha só na acepção de função passiva da atenção; a hipervigilância de AMEAÇA (radar de perigo do TEPT infantil, F3 §3.9) leva sempre o qualificador, porque é outra acepção (G §A2).

**S44 · Cena Bento 2b**
- Título: **A sós, Bento desenha a família enquanto conversa: o exame continua, sem parecer exame.**
- Corpo: imagem única (mesa, papel, lápis). Cena narrada.

**S45 · O desenho como canal**
- Título: **O desenho é a ponte do escolar: num pedido só, você examina afeto, conteúdo do pensamento, cognição e o EU.**
- Corpo: o que pedir (desenho livre, pessoa, família), o que observar (tema, afeto durante o desenho, adequação do traço à idade, se e como a criança se representa), o que registrar (descrever desenho e fluxo em forma narrativa; anexar quando possível). Linha de prontuário de exemplo. Fontes: F1 domínios 13 e 10; F5 (observação durante desenho: relutância, cores, figuras ausentes, autodepreciação, perfeccionismo); F4 (autorretrato).

**S46 · Consciência do EU (escolar)**
- Título: **Dos 7 aos 9 o EU vira psicológico ("eu sou tímido"); perto dos 12, abstrato e cheio de contradições integradas.**
- Corpo: selo VOCÊS JÁ VIRAM (consciência do EU do adulto → a régua etária). A escada com linha de registro por degrau: autodescrição concreta (até ~6) → psicológica e comparativa (7 a 9: "sou melhor em matemática que em leitura") → abstrata e integrada (10 a 12: "tímido com estranhos, extrovertido com amigos"). Fonte: F4 (7-9 e 10-12 anos).
- Narração + alertas de registro: só descrições físicas aos 9; autodesvalorização global ("não gosto de nada em mim") como achado de exame, não desabafo; grandiosidade irrealista; desamparo aprendido ("não adianta tentar"). Autoestima diferenciada por domínios é o esperado; queda global pede nota.

**S47 · Humor vs afeto**
- Título: **Humor é o pano de fundo relatado, afeto é a reação observada: registre um em cada canal, e o afeto em quatro dimensões.**
- Corpo: selo VOCÊS JÁ VIRAM (FND afetividade: registro objetivo + subjetivo; humor como clima, afeto como evento). Humor: relato, com citação textual da criança; polaridade registrada como hipertimia/hipotimia no uso consagrado, NUNCA "distimia" como descritor (colide com a entidade nosológica; G §A6). Afeto: observação em QUALIDADE (tonalidade), AMPLITUDE (normal, restrito, embotado), MOBILIDADE (normal, lábil, fixo) e CONGRUÊNCIA (com o conteúdo). Exemplo de documentação completo nas notas (modelo de F5). Fontes: F5; FND afetividade; F1 domínio 6; G §A6.
- Narração: a dissociação ideoafetiva é o conceito-mãe (a expressão diz uma coisa, o relato diz outra: a criança que sorri ao relatar apanhar; G §A19): vale nota dupla no registro. Paratimia como o nome clássico da incongruência (FND afetividade).

**S48 · Disforia**
- Título: **A depressão da idade escolar muitas vezes não chora: irritabilidade crônica e hostil deve levantar a hipótese de humor.**
- Corpo: contracena de vigilância: disforia na acepção brasileira como cara possível do humor deprimido na criança; alerta de tradução do inglês dysphoria; equivalentes somáticos (dor de barriga, cabeça) no pré-escolar. Fontes: F2 afeto e humor; F5 (manifestações por faixa; irritabilidade e queixas somáticas mais comuns que tristeza verbalizada).

**S49 · Pensamento (curso e forma)**
- Título: **Curso e forma do pensamento se examinam aqui com o vocabulário da aula que vocês já viram, passado pela régua da idade.**
- Corpo: selo VOCÊS JÁ VIRAM (FND pensamento). Curso: aceleração, alentecimento, interrupção/bloqueio (entrada única, G §A5). Forma: fuga de ideias, desagregação, prolixidade (circunstancialidade/tangencialidade), minuciosidade, perseveração (conceito ancorado no pensamento, G §A7/A16), avaliadas na conversa E na narrativa do brincar/desenho. Ressalva de calibração: no pré-escolar essas categorias quase nunca se aplicam; no escolar, sim, com a idade como régua. Fontes: FND pensamento; F2 pensamento; F1 domínio 8; G §A5, §A7, §A13.
- Narração: o critério operacional continua o do curso: o quanto VOCÊ entende a criança classifica a forma; suas sensações (tédio, exaustão, "não entendi") são dado. Rodapé de uma linha: a leitura delirante da interrupção ("roubaram meu pensamento") pertence à consciência do EU, não ao curso do pensamento (G §A5). Sinônimos clássicos (taquipsiquismo, bradipsiquismo) nas notas.

**S50 · Pensamento (conteúdo)**
- Título: **A obsessão tende a chegar como intrusa combatida; o delírio é habitado com convicção: a fronteira é o juízo de realidade.**
- Corpo: escada de conteúdo com linha de registro, descrita em linguagem comportamental (G §0, A1 NÚCLEO): ideia prevalente/sobrevalorada (preocupação desproporcional, questionável), obsessão (repetitiva, intrusiva, incomoda, sentida como pensamento próprio), delírio (convicção incorrigível, fora da cultura e da fase mágica). Linha única sobre delírio infantil (G §0): raro e rudimentar na criança, não sistematizado; quando presente, investigar organicidade. Fontes: F2 pensamento; FND pensamento e juízo/delírio; G §A1 e §A3.
- Rodapé (uma linha, G §A1): tecnicamente a obsessão registra-se como egodistônica E de autoria preservada (o que a distingue da inserção de pensamento); vocabulário pesado, fica na nota. Juízo de realidade = a operação lógica do pensamento sobre o verdadeiro/falso da vivência (G §A3), não um construto separado.
- Nota: em criança o reconhecimento do absurdo é frequentemente parcial ou ausente; o insight não é exigível como no adulto [EXT: ressalva clínica além das fontes].

**S51 · Sensopercepção (epidemiologia)**
- Título: **Diferente da raridade do pré-escolar, o escolar relata vozes com frequência, e a grande maioria (75 a 90%) é transitória.**
- Corpo: UM número em destaque (75 a 90% transitórias). Uma linha de contexto: relatar experiências alucinatórias é mais comum na infância que na vida adulta; prevalência não é psicose. Fonte: F1 domínio 9.
- Narração e notas: Maijer 2018 (9,6% geral; 12,7% aos 5 a 12); Kelleher 2012 (MEDIANA de 17% aos 9 a 12, mediana entre estudos, não teto; 7,5% na adolescência); conversão ~7%. Detalhes no apêndice C.

**S52 · Como eliciar sem contaminar**
- Título: **Pergunte aberto, sem contaminar: "você consegue fazer aparecer? vê e ouve como me vê agora? é pelos ouvidos ou por dentro da cabeça?"**
- Corpo: as três perguntas-modelo na ordem do título, com o que cada uma separa E a linha de registro que cada resposta gera: evocabilidade → fantasia; corporeidade/nitidez → alucinação verdadeira; canal interno → pseudoalucinação. Fontes: F1 domínio 9; F2 sensopercepção.
- Narração: atitude alucinatória (interromper a fala, olhar de escuta para o canto vazio) é o dado OBSERVADO que se registra mesmo quando a criança nega (F2 sensopercepção).

**S53 · Quando a voz vira caso**
- Título: **Em plena vigília, a voz vira caso quando é complexa, angustiante ou incapacitante; com consciência rebaixada, o caso é orgânico.**
- Corpo: contracena de vigilância em dois caminhos: (1) vigília plena + complexidade OU angústia OU prejuízo OU suicidalidade = significância psiquiátrica; (2) rebaixamento ou doença física = investigação orgânica. Fontes: F1 domínio 9 e recomendação 6.

**S54 · Linguagem**
- Título: **A fala do escolar se descreve nos eixos que vocês já usam no adulto: quantidade, velocidade, prosódia, latência, e o silêncio que só existe na escola.**
- Corpo: selo VOCÊS JÁ VIRAM (FND linguagem). Eixos com linha de registro: quantidade (logorreia vs oligolalia/laconismo), velocidade (taquilalia/bradilalia, que não é a mesma coisa que quantidade), prosódia (hipo/hiperprosodia), latência de resposta (entrada única, três contextos, G §2), volume (hipo/hiperfonia). Mais dois observáveis do escolar: pararresposta (resposta que não toca o perguntado) e resposta aproximada (toca sem acertar; a literalidade do TEA entra aqui, G §A4). Destaque: mutismo seletivo (fala em casa, emudece na escola: a topografia do silêncio é o dado) vs mutismo total; ecolalia na idade escolar já sem leitura normativa; neologismo no sentido canônico da linguagem (uso idiossincrático, G §A8). Fontes: FND linguagem (cita o mutismo seletivo como fenômeno da infância); F2 linguagem e fala; G §A4, §A8, §2.

**S55 · Psicomotricidade e vontade**
- Título: **Tique, estereotipia e maneirismo se separam pela função e pelo ritmo, e todos se descrevem antes de se nomear.**
- Corpo: selo VOCÊS JÁ VIRAM (FND psicomotricidade, dona da definição-mãe de estereotipia, G §A14). Trinca com definição de uma linha (tique: súbito, rápido, não rítmico; estereotipia: ação sem sentido nem finalidade, uniforme, de grande frequência, iniciada pelo próprio paciente, com subtipos incluindo o verbal; maneirismo: gesto cotidiano tornado bizarro, com resto de intencionalidade) + a distinção da perseveração motora (repetição de ação que começou certa; conceito ancorado no pensamento, descreve-se o comportamento, G §A16) + vontade e pragmatismo (a criança que quer, começa e não conclui; termo provisório até o extrato de conação, G §4). Fontes: FND psicomotricidade; F2; F3 §3.3; F1 domínio 3; G §A14 e §A16.

**S56 · Risco (a pergunta)**
- Título: **Perguntar de suicídio não planta a ideia: destrava a fonte que muitas vezes é a única que sabe, a própria criança.**
- Corpo: as duas perguntas-modelo por idade + o dado: pais frequentemente desconhecem a ideação; criança em idade escolar também tem e executa comportamento suicida. Fontes: F1 domínio 12; F2 avaliação de risco.

**S57 · Risco (o registro)**
- Título: **Risco se documenta por extenso mesmo quando negativo: ideação, plano, meios, tentativas prévias e o que dispara a raiva.**
- Corpo: o registro-modelo mínimo, campo a campo (ideação passiva vs plano estruturado com acesso a meios; tentativas prévias = preditor estável mais forte; planos/preparativos = preditor agudo; autolesão não suicida distinta de tentativa; heteroagressividade e gatilhos) + a frase-modelo de registro negativo por extenso. Fontes: F1 domínio 12; F2 avaliação de risco.
- Narração: cortes escondidos (antebraço, coxas, moletom no calor). Quebra de sigilo ético-protetiva quando há risco à vida (F2). [EXT: suspeita de maus-tratos exige notificação compulsória (ECA); detalhe no apêndice D.]

**S58 · Fontes múltiplas**
- Título: **Pais veem o que transborda, a criança sente o que não se vê, e o professor compara com trinta da mesma idade.**
- Corpo: divisão de trabalho dos informantes (criança = internalizantes; pais = externalizantes e cronologia; professor = base comparativa) + regra de registro: cada dado com a fonte nomeada. Concordância entre informantes é baixa a moderada; sintoma em dois contextos pesa mais. Fonte: F1 domínio 14.

**S59 · Contraste semiológico ativo**
- Título: **O exame ganha força quando confronta: 12 minutos de foco no consultório contra o relato de que não para 1 minuto em casa.**
- Corpo: a técnica redatorial do contraste (observado no setting vs relatado no ecossistema, lado a lado). Exemplo integral nas notas. Fonte: F3 §4.1.

**S60 · Vigilância do Ato 2**
- Título: **Aos 9 anos, o que tira o sono: a voz que manda, o corte escondido, o silêncio seletivo, a consciência que rebaixa e perder o que já foi conquistado.**
- Corpo: contracena-síntese, espelho do S33, cinco alertas em uma linha cada. Fontes: F1 recomendação 6 e domínios 9 e 12; F2 sensopercepção, risco e consciência.

**S61 · Prontuário 2**
- Título: **Assim se escreve o exame de Bento aos 9: descrição com cena, fonte atribuída e contraste, nenhum rótulo solto.**
- Corpo: o exame psíquico de Bento aos 9, por extenso (10 a 14 linhas), formato narrativo em primeira pessoa (G §A20), incluindo humor/afeto nas quatro dimensões, o EU, o momento a sós, o registro negativo de risco por extenso e a ressalva de representatividade; anatomia da aula de estrutura (introdução → corpo → impressão final bidirecional, o análogo adulto do parágrafo de fechamento; G §5). Ao lado, em miniatura, a MESMA consulta como súmula (o "quê" em lista): o residente vê as duas saídas da mesma observação. Texto integral nas notas e no roteiro. Modelos: F1; F2; F5; FND estrutura; F1 achado 1; G §A20 e §5.

## FECHO (S62 a S65)

**S62 · As armadilhas têm um nome só**
- Título: **Toda armadilha desta aula é a mesma: patologizar o normativo; e a vacina é sempre descrever, calibrar e atribuir.**
- Corpo: as quatro cenas de prontuário ruim revisitadas em miniatura (falso TDAH, falso delírio, falso TOD, falso concretismo) apontando para a tríade-vacina. Fontes: F3 §5; F1 armadilhas; F2 exame cognitivo.

**S63 · Espelho**
- Título: **O mapa não mudou: você acabou de fazer o exame psíquico inteiro que aprendeu neste curso, em outra língua.**
- Corpo: o mesmo mapa do S2, agora com as traduções sobrepostas (discurso→brincar, indivíduo→díade, norma adulta→idade). A aula prova a si mesma.

**S64 · Fronteira**
- Título: **Aos 12, o corpo muda e o exame muda de novo: a sós vira a maior parte da consulta e a confidencialidade vira contrato, mas essa é a próxima aula.**
- Corpo: Bento de costas, mais alto, na mesma porta do S35. Fonte: F1 estrutura por faixa (adolescente).

**S65 · Encerramento**
- Título: **Descreva antes de interpretar, calibre pela idade, atribua cada dado à fonte: o resto vocês já sabiam.**
- Corpo: contato/plataforma + remissão ao apêndice (numeração contínua) e ao roteiro de estudo.

---

## APÊNDICE (satélites, numeração contínua a partir de S66; construção na Camada 3)

- **Apêndice A · ITMSE de Benham (0 a 36 meses) por extenso:** os 10 domínios, um slide cada, com vocabulário observacional. Fonte: F1 seção ITMSE; F3 §3. (~11 slides)
- **Apêndice B · A súmula psiquiátrica infantil (esqueleto do registro + banco de frases-modelo):** o segundo formato de registro (lista, terceira pessoa, o "quê"; G §A20), com a ordem dos campos ancorada nos blocos da aula de estrutura do curso, reordenados por relevância desenvolvimental (dois As da primeira impressão → díade e regulação → brincar/pensamento → bloco das psicoses → bloco orgânico COMA, cuja alteração isolada sugere organicidade também na criança → CPP → bloco do teste recuado; G §5) como checagem de completude, e frases de prontuário prontas, normativas e alteradas, por domínio e faixa, extraídas das tabelas de F2, F3 e dos exemplos de documentação de F5. (~8 slides)
- **Apêndice C · Sensopercepção em números:** Maijer 2018 (9,6%, IC95% 6,7 a 13,6; 12,7% aos 5 a 12), Kelleher 2012 (mediana 17% aos 9 a 12; 7,5% aos 13 a 18), transitoriedade 75 a 90%, conversão ~7%; amigos imaginários (Pearson 2001: 46,2%; Taylor 2004: até 65% antes dos 7). Fonte: F1 domínio 9. (~3 slides)
- **Apêndice D · O REGISTRO do risco e do momento a sós:** foco em documentação, não em roteiro de entrevista. Conteúdo: o que do momento a sós pertence ao exame psíquico (observado + relato da criança com fonte) e o que vai para a anamnese; registro-modelo de risco POSITIVO e NEGATIVO por extenso; registro da autolesão não suicida vs tentativa; como documentar a quebra de sigilo ético-protetiva e a notificação compulsória quando ocorrerem [EXT: ECA], incluindo o valor médico-legal do registro minucioso. As perguntas por idade aparecem apenas como origem do dado registrado. Fontes: F1 domínio 12; F2 risco. (~4 slides)
- **Apêndice E · Anti-pergunta:** respostas prontas para perguntas prováveis (lista fechada na Camada 3; ex.: "e o M-CHAT?", "e quando a escola discorda dos pais?", "amigo imaginário até que idade?", "criança mente no exame?", "posso usar provérbio aos 11?", "e se a criança não brincar nem falar na consulta inteira?", "registro o que a criança me contou a sós se os pais pedirem para ler o prontuário?"). (~5 slides)

**Teste de aceitação da espinha:** ler S1 a S65 só pelos títulos e verificar que a narrativa fecha sem som.
